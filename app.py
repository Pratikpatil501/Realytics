...
def admin_searches():
    if request.headers.get("X-Admin-Secret") != os.environ.get("ADMIN_SECRET","realytics-admin-2025"):
        return jsonify({"error":"Forbidden"}),403
    with get_db() as conn:
        rows=conn.execute("SELECT s.id,s.user_id,s.type,s.inputs,s.result_price,s.region,s.created_at,u.name as user_name,u.email as user_email,u.phone as user_phone,s.guest_id FROM searches s LEFT JOIN users u ON u.id=s.user_id ORDER BY s.created_at DESC LIMIT 200").fetchall()
...
def submit_feedback():
    ...
    if not email or not message: return jsonify({"error":"Email and message are required"}),400
    if not rating or rating<1 or rating>5: return jsonify({"error":"Rating must be between 1 and 5"}),400
    user=current_user()
    try:
        with get_db() as conn:
            conn.execute("INSERT INTO feedback (user_id,user_email,name,email,rating,category,message,created_at) VALUES (?,?,?,?,?,?,?,?)",
                (user["id"] if user else None, user["email"] if user else None, name, email, rating, category, message, datetime.utcnow().isoformat()))
        return jsonify({"ok":True,"message":"Thank you for your feedback!"}),201
    except Exception as e:
        return jsonify({"error":str(e)}),500
...
def admin_feedback():
    if request.headers.get("X-Admin-Secret") != os.environ.get("ADMIN_SECRET","realytics-admin-2025"):
        return jsonify({"error":"Forbidden"}),403
    with get_db() as conn:
        rows=conn.execute("SELECT f.id,f.user_id,f.user_email,f.name,f.email,f.rating,f.category,f.message,f.created_at FROM feedback f ORDER BY f.created_at DESC LIMIT 200").fetchall()
    return jsonify({"feedback":[dict(r) for r in rows],"total":len(rows)})
